...
USAGE() {
  ...
  Options:
    ...
    q     Quick screenshot, saves directly without annotation
...
# screenshot function for quick save
take_quick_screenshot() {
  local mode=$1
  shift
  local extra_args=("$@")

  # execute grimblast with given args and directly save
  if "$LIB_DIR/hyde/grimblast" "${extra_args[@]}" save "$mode" \
    "${save_dir}/${save_file}"; then
    notify-send -a "HyDE Alert" -i "${save_dir}/${save_file}" \
      "saved in ${save_dir}"
  else
    notify-send -a "HyDE Alert" "Screenshot Error" "Failed to take screenshot"
    return 1
...
sc) #? 󱉶 Use 'tesseract' to scan image then add to clipboard
  ...
  pkg_installed imagemagick &&
    magick "${temp_screenshot}" -sigmoidal-contrast 10,50% \
      "${temp_screenshot}"
  tesseract "${temp_screenshot}" - | wl-copy
  notify-send -a "HyDE Alert" "OCR preview" -i "${temp_screenshot}" -e
  rm -f "${temp_screenshot}"
  ;;
q) # quick screenshot without annotation
  take_quick_screenshot "screen"
  ;;
*) # invalid option
  USAGE
  ;;
